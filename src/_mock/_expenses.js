...
export const EXPENSES_SERVICE_OPTIONS = [...Array(8)].map((_, index) => ({
  id: _mock.id(index),
...
const ITEMS = [...Array(3)].map((__, index) => {
  const total = EXPENSES_SERVICE_OPTIONS[index].price * _mock.number.nativeS(index);
  ...
  return {
    ...
    price: EXPENSES_SERVICE_OPTIONS[index].price,
    service: EXPENSES_SERVICE_OPTIONS[index].name,
    quantity: _mock.number.nativeS(index),
  };
});

export const _expenses = [...Array(20)].map((_, index) => {
  ...
  return {
    ...
    totalAmount,
    items: ITEMS,
    expensesNumber: `INV-199${index}`,
    expensesFrom: _addressBooks[index],
    expensesTo: _addressBooks[index + 1],
