...
import { EXPENSES_SERVICE_OPTIONS } from 'src/_mock';
...
export default function ExpensesNewEditDetails() {
  ...
  const handleSelectService = useCallback(
    (index, option) => {
      setValue(
        `items[${index}].price`,
        EXPENSES_SERVICE_OPTIONS.find((service) => service.name === option)?.price
      );
      setValue(
        `items[${index}].total`,
          ...
          <Stack key={item.id} alignItems="flex-end" spacing={1.5}>
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ width: 1 }}>
              ...
              >
                ...
                { EXPENSES_SERVICE_OPTIONS.map((service) => (
                  <MenuItem
                    key={service.id}
                    value={service.name}
